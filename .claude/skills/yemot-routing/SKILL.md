...
### 6.1 "מעבר בסיום" למודול שאין לו הגדרת מעבר (טריק קינון — topic/15214)
בעיה: מודולים מסוימים בימות **אין להם מפתח "מה לעשות בסיום"** — כשהמודול נגמר
המערכת חוזרת אוטומטית **שלב אחד אחורה** לשלוחת-האב, ואי אפשר לכוון אותה ליעד אחר.
עוקפים בקינון שלוש שלוחות:
- **`1/1/1`** = המודול עצמו (זה שאין לו מעבר-בסיום).
- **`1/1`** = מוגדרת כהעברה ל**יעד הסופי** הרצוי (למשל `go_to_folder=/999`).
- **`1`** = מעבירה את המתקשר פנימה ל-`1/1/1`.
הזרימה: המתקשר נכנס ל-`1` → נזרק ל-`1/1/1` (המודול) → בסיום "החזרה שלב אחורה"
מגיעה ל-`1/1` → וזו כבר מנתבת ליעד הסופי. כך "חזרה אחורה" הופכת ל"קדימה ליעד שלנו".
⚠ לא נחוץ ל-`type=api` (שם צד-השרת שולט בניתוב-הסיום). רלוונטי רק כשמכריחים
מודול מובנֶה (השמעת קובץ / סקר / זיהוי) להעביר ליעד אחר בסיום.

### 6.2 פילטר כניסה לשלוחה לפי מספר כשר (topic/16092)
בדיקה אוטומטית של **קידומת נייד כשר** בכניסה לשלוחה, וניתוב לפי התוצאה:

```ini
check_if_mobile_kosher_phone=yes
mobile_kosher_phone_go_to=/1        ; מספר כשר → לכאן
mobile_not_kosher_phone_go_to=hangup ; לא כשר → לכאן
```

- **חל רק על מספרים ניידים ישראליים.** נייחים ושיחות מחו"ל נכנסים **בלי בדיקה**
  (עוברים כרגיל לתוכן השלוחה) — לא נחשבים "לא כשר".
- ערכי היעד: נתיב שלוחה (`/1`), `in_extention` (המשך בשלוחה הנוכחית), או `hangup`.
- ימות מתחזקת את רשימת הקידומות הכשרות (050-41, 052-71/76, 053-31/41, 054-84/85,
  055-32/40/400/401/402/410/52/67/760, 058-32 ועוד — מתעדכנת אצלם).
- ⚠ לא לשימוש בקו שלנו כרגע (הצינתוקים/המענה עובדים על כל המספרים) — תיעוד בלבד.
