...
| I46 | **fetch ריק לא מוחק תשובה שנרשמה** (3.38): `merge_survey_answers` עם `rows` ריק (תקלה זמנית — נטפרי חוסם את workers.dev בזמן ש-77 עונה "אין קובץ"; backoff של 60 שנ'; או מקור שכובה) **לא** ממיר תשובה קיימת (`answer` לא-ריק) ל-"" — `rows_empty` guard מדלג. תשובה קבועה בשרת ⇒ מחיקה = "לא הגיב" שקרי + היעלמות "אישר הגעה" + סנכרון למחשב השני. רק `rows` לא-ריק רשאי לאפס/להזיז תשובה (ריאטריביושן `until_by_phone` דורש שהתשובה תהיה **בתוך** `rows`). מי שעדיין לא ענה כן מסומן "נבדק" (`""`) גם ב-fetch ריק | `yemot.merge_survey_answers` | ✓ |
...
| I47 | **הרשומה נכתבת לפני הדחיפה לשרת המענה** (3.48): `_send` — `add_tzintuk_campaign` + seed **לפני** `_push_list` (שמריצה לולאת-אירועים עד ~10 שנ'). קודם הצינתוק יצא ורק אחרי הדחיפה נרשם — סגירה/קריסה/רענון-סנכרון בתוך החלון הזה = שליחה אמיתית בלי רשומה (שומר-כפילות עיוור, בלי מעקב, בלי היסטוריה). קלאסי ורגיל חולקים נקודת-דחיפה אחת | `_send` | ✓ |
| I48 | **הדוח הסופי לא מאבד מספר שנזרע** (3.48): `yemot.merge_with_seed(seed, reported)` — כל מספר מהזריעה שהתשובה הסופית של השרת לא מזכירה (תשובה ריקה/חלקית, מספר שהשרת דחה בשקט, לא-צולצל אחרי עצירה שהשרת כבר לא מדווח) נשאר ברשומה כ-`pending` (לא-צולצל ⇒ `stopped` אחרי עצירה, מוצע ב"שלח שוב"); `_on_tick` ממזג לפני סימון `stopped` | `yemot.py`/`_on_tick` | ✓ |
| I49 | **המעקב מכבד את הרשומה, לא רק את השרת** (3.48): `_on_tick` קורא את הרשומה בכל טיק — `stopping` שהגיע בסנכרון ⇒ ה-worker מאמץ `stopped_at` מ-`status_ts` (כפתורים נפתחים, סיום אחרי החסד; קודם המחשב השני סקר 60 דק' נעול כי השרת לא מדווח finished אחרי stop); `done` מהמחשב השני ⇒ המעקב נסגר מיד ומצייר מהרשומה בלי לדרוס. **שגיאת-שרת קבועה** (YemotError code>0 ×5, למשל מזהה לא מוכר) ⇒ `permanent` ⇒ `_dead_polls` (בלי חידוש כל דקה שנועל כפתורים 40 שנ' מכל 100 לשבוע), רשומה בת 2+ שעות נסגרת `done` עם הזריעה; רשת/נטפרי (-1/-3) נשארים זמניים | `_on_tick`/`_on_worker_done`/`_maybe_resume_tracking` | ✓ |
| I50 | **החיבור לימות בהגדרות רץ ברקע ו'מחובר' רק אחרי תשובה** (3.49): `_test_yemot_connection` (GetSession ×2 + GetCustomerData + GetApprovedCallerIDs — כל אחת עם retry + שרת תאום, עד ~40 שנ') ובדיקת המזוהה ב-`_save_yemot_settings` (`caller_id_problem`) רצות דרך `_bg`, לא תחת `busy_cursor` — אחרת נטפרי/רשת מתה מקפיאים את החלון לדקות ("לא מגיב"). המזוהה החדש נשמר רק אחרי אישור הקו. הצ'יפ בהגדרות אומר "מחובר" רק אחרי `_probe_yemot_chip` (`_ym_probe_ok`), כמו הצ'יפ בצינתוקים מ-3.22; פרטים שהוקלדו ⇒ "בודק…". כלי `yemot-connect/scripts/connect.py` = קריאה בלבד עם גדר `_guard` על פקודות חיוג | `settings.py`/`connect.py` | ✓ |
