...
### נוסף ב-3.48 (15/9/2026)
- **פעולה חיצונית הצליחה ⇒ רשומה מקומית מיד.** כל צעד אחרי חיוג/תזמון שמריץ לולאת-אירועים (דחיפה לשרת המענה, דיאלוג) בא *אחרי* `add_tzintuk_campaign`. לשאול: "אם התוכנה נסגרת בדיוק כאן — יש רשומה?"
- **תשובה חיצונית ממזגת לתוך הזריעה, לא מחליפה אותה.** לכל כתיבת `report_json` מתשובת שרת: מה קורה עם תשובה ריקה? חלקית? מספר שהשרת השמיט? (`merge_with_seed`).
- **מעקב חי חייב לקרוא את הרשומה, לא רק את השרת.** לכל סטטוס שנשמר ברשומה (`stopping`/`done`/`canceled`) — מה עושה worker שרץ במחשב השני כשהסטטוס מגיע בסנכרון? ולכל כישלון במעקב — זמני (רשת/נטפרי) או קבוע (השרת ענה בשגיאה)? קבוע לא מתחדש כל דקה.
