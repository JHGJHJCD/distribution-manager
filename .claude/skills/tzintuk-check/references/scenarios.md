...
### נוסף ב-3.24 (6/9/2026)
- **מצב שחי רק ב-worker = מת עם התוכנה.** כל דגל שמשנה איך רשומה נסגרת (`stopped_at`) חייב להיות גם ב-DB
  (סטטוס `stopping`) — אחרת סגירה/הפעלה-מחדש/המחשב השני מחדשים את הרשומה בלי הדגל. לכל סטטוס חדש: לעבור על
  `_maybe_resume_tracking`, `answer_windows`, `_list_results`, `tzintuk_campaign_for_date`, `status_he` (היסטוריה+אקסל).
- אחרי עצירה שליחה חדשה מותרת: הרשומה העצורה נשארת `stopping`, ה-poll שלה מפוטר, ומתחדשת (ונסגרת מיד) אחרי שהחדשה מסתיימת.

### נוסף ב-3.25 (6/9/2026)
- **כל תווית-שיפוט על אדם חייבת לשאול "האם בכלל צלצלנו אליו?"** "לא הגיב" נאמר רק על מי שצולצל (`was_rung`). מקורות
  של "לא צולצל": seed `pending` (v3.21), un-dialed אחרי `CampaignAction stop` (מסומן `stopped` בטיק הסופי), ו-`canceled`
  של ימות בלי `startTime`/`redials` (נצפה חי: canceled = ה-redial הממתין בוטל). לכל תווית/מונה חדש — לעבור על
  הטבלה, פירוט ההיסטוריה, `answer_counts` (רצועה+היסטוריה), אקסל, `answer_stats`, ו"שלח שוב".

### נוסף ב-3.28 (7/9/2026)
- **דגל שנגזר מזמן חייב לכסות גם את המסלול ה"מהיר".** `stopped` נולד רק אחרי 10 דק' חסד — אבל השרת יכול לסגור
  את הקמפיין מיד אחרי stop (`finished=True`) ואז אף אחד לא ידע שהייתה עצירה. לכל דגל שמסמן "איך נגמר": לשאול
  "מה אם התנאי הרגיל (`finished`) קורה *לפני* התנאי המיוחד?" — ולבדוק את שני המסלולים בבדיקה מדומה.
- **שני מסלולי קלט לאותה רשימה חייבים לעבור באותו פרסר.** הדבקה כטקסט עברה `find_phones` (שני מספרים בשורה),
  אקסל עבר `normalize_phone_loose` לתא שלם — תא "050…, 052…" נעלם בשקט, בלי "פסול". כל כניסה חדשה של מספרים
  (אקסל/הדבקה/CSV) → `_excel_row_line`/`find_phones`, ובדיקה עם שני מספרים בתא ועם שם שמכיל ספרה.
