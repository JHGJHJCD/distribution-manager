...
- **ברכה בשם (בקשת המשתמש 3/9/2026, נדחה ל"אי-פעם"):** וריאציה מינימלית של
  הזרימה בסעיף 5 — `05xxxxxxx=t-שלום משפחת X.g-/` לכל טלפון של מקבל פעיל, ו-`default=g-/`
  (מי שלא מזוהה ממשיך ישר לתפריט). מומלץ להתחיל בזה כניסוי על שלוחה נפרדת (זול,
  בלי תלות ברשימה השבועית, לא נוגע בשורש) לפני סטטוס-חלוקה.
