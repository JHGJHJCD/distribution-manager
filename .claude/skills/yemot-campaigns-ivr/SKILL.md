...
- **זיכרון "כבר שמע" (אומת חי 3/9/2026):** המונה (`-1-6d`) נשמר **לכל מתקשר** בקובץ
  ...
  מחזיר שגיאה כללית "error"). מנהל חלוקה מוחק את הקובץ שלו בכל שליחה
  (`yemot.reset_callback_memory`) כדי שכל נמען ישמע את ההודעה החדשה פעם אחת. **החלפת ההקלטה
  או הרשימה בתבנית לא מאפסת את המונה** — רק מחיקת הקובץ.
