...
- קובץ מצורף שנמחק מהדיסק לפני השליחה → אזהרה ב-`_send` (M35) **וגם** ב-`_send_test` (M39, 3.44).
- ניתוק/timeout באמצע `sendmail` (SMTP) → עברית "נותק באמצע השליחה", לא-fatal (M38). **מ-3.47:** חיבור *ותיק* שנפל → חיבור-מחדש וניסיון נוסף לאותו נמען (M50); חיבור טרי שנפל = כישלון אמיתי.
- 500 נמענים ב-SMTP → חיבור+login **אחד** (`SmtpSession`), לא 500 (Gmail: "Too many login attempts") (M50).
- התקנת עדכון-תוכנה באמצע שליחה → `guard_update` שואל לפני ההורדה ולפני ההחלפה (quit() עוקף את closeEvent) (M51).
- "שלח שוב לנכשלים" כשבטיוטה מצורף קובץ *אחר* → נשלח הקובץ **המקורי** של הרשומה; קובץ מקורי חסר → שאלה (M49).
- "שמור כתבנית" בשם של תבנית קיימת → שאלה + עדכון הקיימת, לא כפילות (M52).

### היסטוריה וסנכרון
- שליחה במחשב A → מופיעה ב-B עם `sent`/`failed` (`mail_add` ואז `mail_update`).
- B מקבל `mail_update` לפני `mail_add`? — **לא קורה**: שניהם נכתבים ליומן של אותו מחשב (או ל-outbox שלו) בסדר, ו-`pull_changes` מחיל לפי offset. הוכרע 14/9/2026 (v3.44).
- **"שלח שוב לנכשלים" / כרטיס מקבל על שליחה של המחשב השני** → `rec_id` בדוח הוא id **מקומי** של המחשב ששלח; ב-B אותו מספר = אדם אחר. התאמה רק לפי `guid` (M37, `test_mail.py` §8).
...
- `_secret.py` קיים ב-EXE אבל המשתמש טען קובץ אחר בהגדרות → `_secret` מנצח (סדר M14) — **הכרעת המשתמש 14/9/2026: כן, המובנה גובר.**
