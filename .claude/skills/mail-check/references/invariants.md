...
| M7 | שליחה שנקטעה (תוכנה נסגרה) נסגרת כ-`interrupted`: של המחשב הזה — מיד בהפעלה; של המחשב השני — **רק** אחרי `STALE_PEER_HOURS` (24 שעות) בלי עדכון `status_ts` (סקירת בשלות 26/9/2026: שורה של מחשב שקרס נשארה "⏳ נשלח עכשיו…" לנצח בלי 'שלח שוב לנכשלים'; ה-LWW של המחשב ההוא דורס אם בכל זאת סיים); ו**לא** של השליחה הפעילה (`_active_guid`) (3.39) | `mails._close_stale_campaigns` | ✓ |
...
| M37 | **`rec_id` בדוח הוא מזהה מקומי** (3.44): במחשב השני אותו מספר = אדם אחר. `get_mails_for_recipient(rec_id, guid)` מתאים לפי `guid` כשהשורה נושאת אותו; `_resend_failed` פותר כרטיס דרך `get_recipient_by_guid`, ו-`rec_id` רק כששליחה היא של המחשב הזה (`device == me`) — אחרת יעד חיצוני לכתובת שבדוח. בלי זה "שלח שוב לנכשלים" מהמחשב השני שלח לאדם הלא-נכון, וכרטיס מקבל הציג מייל של מישהו אחר | `database.get_mails_for_recipient`/`mails._resend_failed`/`search.py` | ✓ |
| M38 | SMTP: ניתוק/timeout **באמצע** `sendmail` (אחרי חיבור מוצלח) נתפס (`OSError`/`SMTPException`) → עברית "נותק באמצע השליחה", לא-fatal (הנמען הבא ייכשל בחיבור ויעצור) (3.44) | `email_utils.send_email` | ✓ |
| M39 | `_send_test` בודק שהקובץ המצורף קיים בדיסק, כמו `_send` (M35) — מייל-בדיקה בלי הקובץ הטעה את המפעיל שהכל תקין (3.44) | `mails._send_test` | ✓ |
| M45 | **סגירת התוכנה באמצע שליחה** (3.46): `MainWindow.closeEvent` → `mails_tab.sending_active()` → `confirm_close()` (ברירת-מחדל "לא"); אישור → `_abort_for_close`: `stop()` + `wait()` למייל הנוכחי + `processEvents` (ה-progress שבתור) + הרשומה נסגרת כ-`interrupted` דרך `close_pending` **מיד ובסנכרון** — לא "בתהליך" עד ההפעלה הבאה, ולא סגירה שקטה של שליחה ל-500 | `main.closeEvent`/`mails.confirm_close`/`_abort_for_close` | ✓ |
| M46 | **Gmail API — בקשת JSON מוגבלת ל-10MB** (3.46): `raw` = base64 של MIME שכבר מכיל base64 של הקובץ ⇒ קובץ 6MB חורג. מעל `JSON_RAW_LIMIT` (5MB) → `GMAIL_UPLOAD_URL` (`uploadType=media`, `Content-Type: message/rfc822`, עד 35MB, בלי ניפוח). 413/"too large"/"payload size" = **fatal** — גודל ההודעה לא תלוי בנמען, אחרת 500 העלאות-ענק שנכשלות | `google_auth.gmail_send_raw` | ✓ |
| M47 | **גודל המייל נבדק בתוכנה לפני כל שליחה** (3.46): `send_email` → `len(mime_bytes) > MAX_MAIL_BYTES` (25MB, מגבלת Gmail אחרי קידוד) ⇒ `MailFatalError` בלי בקשת רשת; תקרת הקובץ במסך = `email_utils.MAX_ATTACHMENT_BYTES` = 18MB (20MB×1.37 = 27MB היה עובר את המסך ונכשל אצל כולם) | `email_utils.send_email`/`mails._pick_attachment` | ✓ |
| M49 | **שליחה-חוזרת עם הקובץ המקורי** (3.47): `mail_campaigns.attachment` + `with_header` נשמרים ברשומה (DB + `mail_add` + seed + `_apply_mail_add`); `_send(attachment=, with_header=)` מפורשים; `_resend_failed` לוקח אותם **מהרשומה** — לא מהטיוטה (קודם: "שלח שוב לנכשלים" צירף את הקובץ שהיה *כרגע* במסך — קובץ אחר לגמרי, או בלי קובץ). קובץ מקורי חסר (נמחק / מחשב שני) → שאלה עם ברירת-מחדל "לא", לא שליחה בשקט. נכשלים שאיבדו כתובת תקינה → הודעה כמה לא יישלח | `mails._send`/`_resend_failed`, `database.add_mail_campaign`, `sync.py` | ✓ |
| M50 | **SMTP: חיבור+login אחד לכל האצווה** (3.47): `email_utils.SmtpSession` (`mail_session()`), `send_batch` פותח אחד ומעביר `session=`; `send_email` בלי session מתחבר ומנתק בעצמו (מייל-בדיקה / מתנדבים). ניתוק של חיבור **ותיק** באמצע `sendmail` → `reset()` וניסיון נוסף אחד לאותו נמען; חיבור **טרי** שנפל = כישלון אמיתי (M38). בלי זה: 500 כניסות רצופות ⇒ Gmail "454 4.7.0 Too many login attempts" באמצע הרשימה | `email_utils.send_email`/`SmtpSession`, `mailer.send_batch` | ✓ |
| M51 | **עדכון-תוכנה באמצע שליחה** (3.47): ההתקנה יוצאת ב-`QApplication.quit()` שלא עובר ב-`closeEvent` (M45 לא תופס) ⇒ `settings._start_download` **ו-**`_on_downloaded` (לפני `apply_update`, כי השליחה יכלה להתחיל בזמן ההורדה) שואלים דרך `MailsTab.guard_update()` (= `confirm_close`, ברירת-מחדל "לא") | `settings._mails_guard_ok`, `mails.guard_update` | ✓ |
| M52 | "שמור כתבנית" בשם של תבנית **קיימת** (לא זו שנבחרה) → שאלה ועדכון אותה תבנית (guid שלה), לא תבנית שנייה באותו שם (3.47) | `mails._save_template` | ✓ |
| M48 | **שליחה-חוזרת לא דורסת את הטיוטה** (3.46): `_send(targets, audience, subject=, body=)` — `_resend_failed` מעביר את נושא/גוף הרשומה במפורש ולא כותב לשדות (קודם: ביטול בחלון האישור = הטיוטה שהמפעיל כתב נמחקה). **"התנתק" מגוגל** = `disconnect(revoke=False)` מקומי + `revoke_token` ב-`_BgWorker` — קריאת-הרשת לא על ה-UI (M10) | `mails._send`/`_resend_failed`, `settings._google_disconnect` | ✓ |
