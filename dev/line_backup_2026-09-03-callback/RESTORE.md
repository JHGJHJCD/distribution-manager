...
2. **שלוחה חדשה `/78` "הודעת החלוקה (מנהל חלוקה)"** — `type=menu`, `digits=2`, `timeout=1`, `timeout_goto=/2`.
   ...
   **גרסה 3.07 (אותו יום, 14:13):** ב-78 נוסף אקסס-פילטר — `check_access_filter=yes`,
   `access_filter_1=g.*.*.*.*.*.30d.1.30d` (מעבר אחד ל-30 יום למספר), `access_filter_no_goto=/78/menu`;
   ותת-שלוחה **`/78/menu`** (menu, `M0000.wav` = פתיח השורש בלבד, אותם 20 קישורים) למי שכבר שמע.
   הזיכרון `78/AccessFilterLogTime.ini` נמחק ע"י התוכנה בכל שליחה.
