...
## לשוניות בפועל — 6 (סדר נשמר בגרירה לפי מפתח יציב ב-`tab_order`)
מוגדרות ב-`main.py` `tab_specs`. הסדר: חלוקה ורישום · מקבלים · חד פעמי · חיפוש מהיר · חלוקות · הגדרות.
- `tabs/group_update.py` — **"חלוקה ורישום"** (לשונית פתיחה; מיזוג של weekly+group). **מרובה מוצרים** (`widgets.ProductsEditor` — טבלת כמות|סוג פריט|מחק, כל שורה עם "כמות לאדם"; נשמר כמחרוזת ב-`what_dist`) + **הערה כללית לחלוקה**. **חיפוש מהיר** מסנן את רשימת השבוע לפי כל שדה (`db.filter_recipients`, `_search_text`/`_apply_search`) — החליף את scope+area combos. סימון מוחזק ב-`self._checked_ids` (שורד סינון/הסתרה) + `_seen_ids` לסימון-מראש של חד-פעמיים חדשים. רישום → `bulk_add_distributions(..., dist_name, general_note)` שגם יוצר שורת אצווה. **זרימת מתנדבים:** `_dispatch_volunteer_email` / `_import_volunteer_results`.
  **עיצוב 2026 (v2.37+):** לשונית זו בנויה מכרטיסים לבנים (`_make_card`/`_field` helpers) על רקע אפור `#f5f7fb`, כפתורים כחול/ירוק/אדום עקביים (`_BTN_PRIMARY/_SUCCESS/_DANGER/_GHOST`), אייקוני-קו (`utils.ui.line_icon` — הורחב עם `doc/box/mail/send/upload/download/print/plus/trash`), וסרגל פעולות תחתון קבוע (שמור/יצוא/הדפסה). כרטיסי הפרטים+מוצרים+מתנדב עטופים ב-`QScrollArea` פנימי (`top_scroll`) כדי שרשימת המקבלים (`list_card`, stretch=1) תמיד תקבל את רוב הגובה ולא תיסחט במסכים נמוכים — זה ויזואלי בלבד, אין שינוי בשמות ה-widgets/החיבורים/הלוגיקה. **שים לב:** אל תשתמש ב-`QGraphicsDropShadowEffect` על כרטיס בתוך layout גמיש — זה שובר את משא-ומתן הגובה המינימלי ומכווץ את התוכן (גילינו זאת בקושי בסשן הזה); אם צריך צל, זה חייב טכניקה אחרת.
...
- `tabs/distributions.py` — **"חלוקות"** (v2.37). שורה אחת לכל אצווה מטבלת `dist_batches` (תאריך/שם/מוצרים/כמות/מחלק/קיבלו/נפשות/הערה כללית). לחיצה כפולה → `BatchDetailsDialog` (מי קיבל + הערה מלאה); כפתור מחיקה (`db.delete_batch`).
...
- **קוד מת שנמחק/מנותק:** `tabs/weekly.py` ו-`tabs/tracking.py` נמחקו; `tabs/summary.py` לא לשונית/לא מחובר (אל תסתמך עליו). `review.py` נפתח כדיאלוג בלבד.
- **מבנה (v2.37):** לוגיקת הניקוד ב-`scoring.py` (מודול טהור); `database.py` מייבא-מחדש את סמליו (`db.NEED_FACTORS` וכו'). סקריפטי פיתוח (probe/screenshot/benchmark/stress/create_icon) עברו ל-`dev/` עם shim ל-project-root; קובצי `test_*.py` נשארו בשורש.
...
- `scoring.annotate_need_scores(rows, weights)` (מודול טהור; `database._annotate_need_scores` = עטיפה שמושכת `get_need_weights`): ניקוד 0–100 = שקלול 6 גורמים (`NEED_FACTORS`). משקלים מתכווננים בהגדרות (`need_w_*`, מסתכמים ל-100). מאכלס `_score_parts` לפירוט בלחיצה.
