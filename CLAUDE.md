...
- **צינתוק קלאסי (בחירה בכל שליחה):** `_SendModeDialog` מחליף את חלון האישור — רדיו "עם הודעה"/"קלאסי". קלאסי = `run_campaign(classic=True)`: תבנית שנייה (`ensure_classic_template`, setting `yemot_classic_template_id`; מאמצת את התבנית היתומה 1430693, מוגדרת חד-פעמית `originateTimeout=8`+`maxDialAttempts=1`), ההקלטה מועתקת אליה בצד השרת, הרשימה נשמרת **בתבנית הראשית** (ההתקשרות-חוזרת מושמעת דרך `campaign_message_to_play` מיקום 17). אין מעקב-מענה בקלאסי — נרשם `done` מיד עם קידומת "צינתוק קלאסי —".
