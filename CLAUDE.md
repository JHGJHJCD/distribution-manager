...
- **נטפרי חוסם צ'אטים של Claude על תמונות (1/9/2026, 3 צ'אטים אבדו):** נטפרי סורק את גוף הבקשות ל-`api.anthropic.com` **כולל תמונות**; `Read` של PNG מסוים (קרה עם `dev/shots_classic_track/*.png`) יכול להקפיץ 418 "badwords" — ומאותו רגע הצ'אט מת לצמיתות (התמונה נשלחת מחדש בכל בקשה), וגם "המשך שיחה" יורש אותה ונחסם מיד. **כלל:** אם צ'אט נחסם מיד אחרי קריאת צילום — לא לקרוא את הקובץ שוב בשום צ'אט; לאמת צילומים חשודים דרך `gemini_task.py -f <png>` (ראיית Gemini, רק טקסט חוזר), ואחרי חסימה לפתוח צ'אט חדש נקי מ-`NEXT_TASK.md`, לא "המשך".
