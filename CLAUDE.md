...
- **משמעת שחרור:** עדכן `version.py` `APP_VERSION` → בנה → `cp dist/מנהל_חלוקה.exe dist/Manhal-Haluka.exe` (asset ב-ASCII) → `git add/commit/push` → `gh release create vX.Y dist/Manhal-Haluka.exe --latest`. **לפרסם רק Release** — לא לגעת בתיקיות/ZIP בשולחן העבודה. הודעת commit מסתיימת ב-`Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>`.
