...
class FeedbackDialog:
    ...
    ההודעה נשמרת דרך utils.feedback.save_feedback (JSONL מקומי + טבלת feedback
    מסונכרנת — מוצגת ב"הודעות שנשלחו" בהגדרות) ויוצאת למפתח **במייל בלבד**
    (#u7gmi, 23/9/2026): ישירות מחשבון המייל המחובר, ואם אין — נפתחת תוכנת
    המייל של המשתמש עם "דיווח תקלה (מנהל חלוקה)" מוכן. GitHub/טופס Google הוסרו.
    """
    @staticmethod
    def open(parent=None):
        ...
        # #u7gmi: the message goes to the developer by MAIL only — straight from
        # the app when a mail account is connected, otherwise via the user's own
        # mail program with a ready-made 'דיווח תקלה (מנהל חלוקה)' message.
        _mail_ready = email_utils.is_configured()
        how = QLabel("ההודעה תישלח למפתח ישירות מחשבון המייל של הקופה ✓" if _mail_ready
                     else "אין חשבון מייל מחובר — תיפתח לך תוכנת המייל עם ההודעה מוכנה לשליחה.")
        how.setWordWrap(True)
        how.setStyleSheet("color:%s; font-size:12px; font-weight:700;" % ("#047857" if _mail_ready else "#b45309"))
        lay.addWidget(how)
        ...
        def _do_send():
            ...
            if _mail_ready:
                ...
                if ok:
                    ...
                    dlg.accept()
                    return
                QMessageBox.warning(dlg, "שליחה במייל נכשלה",
...
def load_changelog() -> list:
    """Read the bundled changelog.txt (generated by dev/gen_changelog.py from
    git history) → [(version, notes)] newest first. Lives next to sys._MEIPASS
    in the frozen EXE, else at the repo root. Missing/unreadable → []."""
    import os
    import sys
    base = getattr(sys, "_MEIPASS", None) or os.path.abspath(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), ".."))
    path = os.path.join(base, "changelog.txt")
    try:
    ...
    if ver is not None:
        out.append((ver, "\n".join(buf).strip()))
    return [(v, n) for v, n in out if v]


class ChangelogDialog(QDialog):
    """'יומן שינויים' — every released version with its notes, one white card
    per version, in the visual language of UpdateOfferDialog."""

    def __init__(self, parent=None, entries=None, current_version: str = ""):
        super().__init__(parent)
        if entries is None:
        ...
        self.setWindowTitle("יומן שינויים")
        self.setLayoutDirection(Qt.LayoutDirection.RightToLeft)
        self.setSizeGripEnabled(True)
        self.setMinimumSize(520, 420)
        scr = QApplication.primaryScreen()
        max_h = int(scr.availableGeometry().height() * 0.9) if scr else 640
        self.resize(760, min(640, max_h))
        self.setStyleSheet("QDialog{background:#f4faf7;}")
        ...
        hl.setContentsMargins(22, 18, 22, 16)
        hl.setSpacing(4)
        t1 = QLabel("📜 יומן שינויים — מנהל חלוקה")
        t1.setStyleSheet("color:#ffffff; font-size:18px; font-weight:800; background:transparent;")
        sub = ("הגרסה המותקנת: " + current_version + " · " + str(len(entries)) + " גרסאות"
               if current_version else str(len(entries)) + " גרסאות")
        t2 = QLabel(sub)
        t2.setStyleSheet("color:rgba(255,255,255,0.92); font-size:13px; font-weight:600;"
                         " background:transparent;")
        hl.addWidget(t1)
        hl.addWidget(t2)
        lay.addWidget(head)
        ...
        self.cards = []
        if not entries:
            empty = QLabel("אין עדיין רשומות ביומן השינויים.")
            empty.setAlignment(Qt.AlignmentFlag.AlignCenter)
            empty.setStyleSheet("color:#5f7a70; font-size:13px; padding:30px;")
            bl.addWidget(empty)
        for ver, notes in entries:
            card = self._card(ver, notes, ver == current_version)
        ...
        foot = QFrame()
        foot.setStyleSheet("QFrame{background:#ffffff; border-top:1px solid #d7ebe2;}")
        fl = QHBoxLayout(foot)
    ...
    @staticmethod
    def _card(ver: str, notes: str, current: bool) -> QFrame:
        card = QFrame()
        card.setStyleSheet(
            "QFrame#clcard{background:#ffffff; border:1px solid "
            + ("#0f9d78" if current else "#d7ebe2") + "; border-radius:12px;}")
        ...
        top = QHBoxLayout()
        top.setSpacing(8)
        badge = QLabel("גרסה " + ver)
        badge.setStyleSheet(
            "QLabel{background:#e2f6ee; color:#0f766e; font-size:13px; font-weight:800;"
            " border-radius:10px; padding:3px 12px; border:none;}")
        top.addWidget(badge)
        if current:
            cur = QLabel("הגרסה הנוכחית")
            cur.setStyleSheet(
                "QLabel{background:#0f9d78; color:#ffffff; font-size:11.5px; font-weight:700;"
                " border-radius:10px; padding:3px 10px; border:none;}")
            top.addWidget(cur)
        top.addStretch(1)
        cl.addLayout(top)

        txt = QLabel(_release_notes_html(notes) or "<p dir='rtl'>—</p>")
        txt.setTextFormat(Qt.TextFormat.RichText)
        txt.setWordWrap(True)
        txt.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse)
        txt.setAlignment(Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignRight)
        txt.setStyleSheet("QLabel{background:transparent; color:#334155; border:none;}")
        cl.addWidget(txt)
        return card

    @staticmethod
    def open(parent=None):
        dlg = ChangelogDialog(parent)
        dlg.exec()
        return dlg
