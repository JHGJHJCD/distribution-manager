...
class MailsTab(QWidget):
    ...
    def _build_ui(self):
        ...
        self.btn_add_addr.setToolTip("הוספת כתובת למי שאינו רשום כמקבל — מתנדב, חבר הנהלה או תורם")
        ...
        self.btn_show_bad.setToolTip("מציג רק את מי שלא יקבל מייל · לחיצה כפולה על שורה פותחת את הכרטיס להוספת כתובת")
        ...
        self.btn_preview_next.setToolTip("הצגת ההודעה כפי שתיראה אצל הנמען הבא ברשימה")
        ...
        self.btn_test.setToolTip("שליחת ההודעה לכתובת של חשבון הקופה, עם השם של הנמען הראשון")
    ...
    def _refresh_account(self):
        ...
        self.chip_with_mail.setToolTip("כמה מהמקבלים הפעילים רשומים בכרטיס עם כתובת מייל")
    ...
    def _fill_table(self):
        ...
        for i, tg in enumerate(self._targets):
            ...
            if fixable:
                st.setToolTip("לחיצה פותחת את כרטיס המקבל — להוספה או תיקון של כתובת המייל")
            t.setItem(i, 2, st)
            ...
            rm.setToolTip("הסרה מהשליחה הזו בלבד — הכרטיס עצמו לא נמחק")
            t.setItem(i, 3, rm)
